<template>
    <div class="container">
        <div class="p-2 pageHeading" v-if="formType =='acc_rec_update'">Account Receivable - Update</div>
        <div class="p-2 pageHeading" v-else>Receivable by Age</div>
        <div class="row">
        ...
        <div v-else class="col pb-2">
            <table class="table">
                ...
                <tbody v-if="formType == 'acc_rec_update'">
        ...
        <show-payment-details ref="showPaymentDetails" :order-info="selectedItem" @reload:orders="getAllOrders" />
    </div>
</template>
    ...
    export default {
        ...
        data(){
            return {
                ...
                selectedItem:{},
                tableHeader: '',
                tableHeaderUpdate:["Order No.", "Order Date", "Customer", "Make & Model", "Balance"],
                tableBody: [],
                tableHeaderAged:["Order date", "Aged", "Order No.", "Customer", "Total", "Amount Paid", "Balance"],
                invoices: [],
                loading: false,
                subTotal: 0,
                totalAmountPaid: 0,
        ...
        async mounted() {
            this.tableHeader = this.formType == "acc_rec_update" ? this.tableHeaderUpdate : this.tableHeaderAged;
            ...
            resultQuery() {
                if(this.searchQuery){
                    return this.tableBody.filter((item)=>{
                        return this.searchQuery.toLowerCase().split(' ').every(v => (item.first_name.toLowerCase().includes(v) || item.last_name.toLowerCase().includes(v) || item.model_name.toLowerCase().includes(v)))
                    })
                }else{
