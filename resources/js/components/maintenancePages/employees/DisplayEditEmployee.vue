<template>
    <div v-if="displayEditEmployee">
        <div id="modal">
            <div class="modal-content">
                <div class="pageHeading">
                    View/Edit Employee<span class="closeModal pull-right" @click="hide">&times;</span>
                </div>
...
export default {
    ...
    computed:{
        disableUpdateButton() {
            return Object.entries(this.employeeInfo).toString() === Object.entries(this.formFields).toString() || this.formFields.phone == '' || this.formFields.first_name == '' || this.formFields.last_name == '' || this.formFields.address == '' || this.formFields.state == '' || this.formFields.city == '' || this.formFields.zip == '' || this.formFields.start_date == '';
        },
    },
    methods:{
        acceptNumber(value) {
        ...
        async update(){
            ...
            try {
                axios.post(`updateGivenEmployee`, this.formFields);
            } catch (exception) {
        ...
        show() {
            this.formFields = JSON.parse(JSON.stringify(this.employeeInfo));
